...
if (false == m_start)
    {
        if(0 == ControlBase::StartTasks())
        {
            ...
            m_scopeLightsDataTask_attributes.stack_size = 1024;
        ...
        if (NULL != m_scopeLightsDataTaskHandle)
        {

           return 0;
        }

        if (NULL != m_chamberLightsDataTaskHandle)
        {
...
bool ControlScopeLight::ProcessCommands(ControlCommandData& commandData)
{
    ...
    {
        if (commandData.isCommand(MCU_CMD_LIGHT_CONTROL))
        {
                if (ProcessCommandScope(commandData))
                    return true;
        }

        if (commandData.isCommand(MCU_CMD_CHAMBER_CONTROL))
        {
                if (ProcessCommandChamber(commandData))
                    return true;
        }
    }
    return false;
...
bool ControlScopeLight::ProcessCommandScope(ControlCommandData& commandData)
{
    if(false == ControlBase::ProcessCommands(commandData))
    {
        if (2 == commandData.paramSize())
        {
            if (commandData.paramValid(1))
            {
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_ON))
                {
                    ScopeLightsOn();
                    m_led = true;
                    return true;
                }
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_RED))
                {
                ...
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_GREEN))
                {
                    m_scopeRed = SCOPE_LIGHTS_OFF;
                    m_scopeGreen = SCOPE_LIGHTS_ON;
                    m_scopeBlue = SCOPE_LIGHTS_OFF;
                ...
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_BLUE))
                {
                    m_scopeRed = SCOPE_LIGHTS_OFF;
                    m_scopeGreen = SCOPE_LIGHTS_OFF;
                ...
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_WHITE))
                {
                    m_scopeRed = 150;
                    m_scopeGreen = 80;
                    m_scopeBlue = 100;
                ...
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_YELLOW))
                {
                    m_scopeRed = 250;
                    m_scopeGreen = 160;
                    m_scopeBlue = 15;
...
//                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_PINK))
//                                {
//                                    m_scopeRed = 150;
//                                    m_scopeGreen = 20;
//                                    m_scopeBlue = 15;
...
//                                }
                ...
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_OFF))
                {
                   ScopeLightsOff();
                   m_led = false;
                   return true;
                ...
                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_ENABLE))
                {
                    EnableTouchSensor();
                    return true;
                }

                if (0 == strcmpi(commandData.getParam(1), MCU_CMD_DISABLE))
                {
                    DisableTouchSensor();
                    return true;
                }
        ...
        else if (4 == commandData.paramSize())
        {
...
bool ControlScopeLight::ProcessCommandChamber(ControlCommandData& commandData)
{
    if (2 == commandData.paramSize() || 3 == commandData.paramSize())
    {
        if (commandData.paramValid(1))
        {
            if (0 == strcmpi(commandData.getParam(1), MCU_CMD_ON))
...
void ControlScopeLight::NeoPixelShow()
{
    ...
    HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_3, m_dmaBuffer, DMA_BUFF_SIZE);
